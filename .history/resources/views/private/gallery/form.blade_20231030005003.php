...
@section('navbar')
    @if (!isset($gallery)) Tambah Gallery
    @else Edit Gallery
    @endif
@endsection
        ...
        <form id="practice_form" class="space-y-4">
            ...
            <div>
                <label role="button" id="image_label" for="image_input" class="image_form w-full border hover:bg-gray-100 border-blue-900 relative overflow-hidden p-4 h-72 rounded-xl flex items-center justify-center">
                    ...
                    @if (isset($gallery))
                        <img id="image_preview" src="/assets/images/{{ $gallery->image }}" class="absolute image_preview w-full h-full object-image object-center" alt="">
                    @else
            ...
            <input id="id_input" value="{{ !isset($gallery) ? '' : $gallery->id }}" type="hidden">
            <div class="form-control">
                <label for="name_input" class="block mb-2">Nama Kegiatan</label>
                <input id="name_input" value="{{ !isset($gallery) ? '' : $gallery->name }}" type="text" required maxlength="42" class="input input-bordered w-full" placeholder="cth. Informasi Update Terkini Di Kota Tulungagung">
            </div>
            <div class="form_control">
                <label for="description_input" class="block mb-2">Deskripsi Profil HWLawyer</label>
                <textarea id="description_input" maxlength="100" required class="textarea w-full textarea-bordered h-32" placeholder="cth. Didirikan pada tahun 2021, HW Lawfirm membawa tradisi 17 tahun keahlian dalam dunia hukum...">{{ !isset($gallery) ? '' : $gallery->name }}</textarea>
            </div>
            <div class="w-full mt-3 flex justify-end justify-items-end items-center gap-x-3">
                <a href="/private/galleries" class="btn btn-ghost border border-gray-300">Batal</a>
...
document.getElementById('practice_form').addEventListener('submit', function(e) {
    ...
    let endpoint = ''
    if(id == '') endpoint = '/private/gallery/store'
    else endpoint = '/private/gallery/update/'+id
