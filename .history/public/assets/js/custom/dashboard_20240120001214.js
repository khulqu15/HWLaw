...
function uploadImageContent(event) {
    ...
    const formData = new FormData()
    if(logo.files[0]) formData.append('cover_image', logo.files[0])
    if(area.files[0]) formData.append('image1', area.files[0])
    if(about.files[0]) formData.append('image2', about.files[0])
    if(contact.files[0]) formData.append('image2', contact.files[0])
    fetch('/private/about/upload_image', {
        method: 'POST',
        body: formData,
    })
