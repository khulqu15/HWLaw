...
<div class="grid md:grid-cols-2 grid-cols-1 gap-x-8 relative items-center md:p-20 p-12 bg-blue-900 relative bg-fixed bg-cover bg-center" style="background-image: url('/assets/images/6.jpg')">
    ...
    <div class="relative text-white">
        <p class="text-yellow-400 mb-4 tracking-wider uppercase md:pt-0 pt-12">AREA PRAKTEK KAMI</p>
...
<div class="relative w-full bg-white pr-12 md:pl-0 pl-12 py-12">
    ...
    <div class="grid grid-cols-12 gap-6 justify-center justify-items-center mt-5">
        <div class="lg:col-span-5 md:col-span-7 col-span-12 w-full h-96 bg-blue-500 text-white md:rounded-l-none rounded-l-xl rounded-r-xl overflow-hidden relative">
            ...
            <div class="w-full py-6 px-6 absolute left-0 bottom-0 bg-gradient-to-t from-black to-transparent">
                <h3 class="md:text-xl text-lg font-bold mb-2 break-words">{{ $post[0]->title }}</h3>
                <div class="flex gap-x-3 items-center mb-2">
                    <div class="flex items-center gap-x-1">
                        <p class="text-xs">{{ $post[0]->category->name }}</p>
                    </div>
                    <div class="flex items-center gap-x-1">
                        <iconify-icon class="text-lg" icon="solar:calendar-date-bold"></iconify-icon>
                        <p class="text-xs">{{ date('d MM YY', strtotime($post[0]->date)) }}</p>
                    </div>
                    <div class="flex items-center gap-x-1">
                        <iconify-icon class="text-lg" icon="heroicons:map-pin-20-solid"></iconify-icon>
                        <p class="text-xs">{{ $post[0]->location }}</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="lg:col-span-4 md:col-span-5 col-span-12 space-y-6 w-full">
            <div class="w-full h-[11.25rem] overflow-hidden bg-blue-900 rounded-xl relative">
                <img src="/assets/images/{{ $post[1]->cover }}" class="w-full relative h-full object-cover object-center" alt="HW Lawyer Berita 1">
                ...
                <div class="w-3/4 left-1/4 py-6 px-6 absolute text-white left-0 top-0 h-full bg-gradient-to-l from-black to-transparent">
                    <h3 class="text-lg font-bold mb-2">{{ $post[1]->title }}</h3>
                    <div class="flex gap-x-3 items-center mb-2">
                        <div class="flex items-center gap-x-1">
                            <p class="text-xs">{{ $post[1]->category->name }}</p>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="solar:calendar-date-bold"></iconify-icon>
                            <p class="text-xs">{{ date('d MM YY', strtotime($post[1]->date)) }}</p>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="heroicons:map-pin-20-solid"></iconify-icon>
                            <p class="text-xs">{{ $post[1]->location }}</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full h-[11.25rem] overflow-hidden bg-blue-900 rounded-xl relative">
                <img src="/assets/images/{{ $post[2]->cover }}" class="w-full relative h-full object-cover object-center" alt="HW Lawyer Berita 1">
                <div class="w-full h-full top-0 absolute left-0 bg-gradient-to-l from-black to-black/20"></div>
                <div class="w-3/4 left-1/4 py-6 px-6 absolute text-white left-0 top-0 h-full bg-gradient-to-l from-black to-transparent">
                    <h3 class="text-lg font-bold mb-2">{{ $post[2]->title }}</h3>
                    <div class="flex gap-x-3 items-center mb-2">
                        <div class="flex items-center gap-x-1">
                            <p class="text-xs">{{ $post[2]->category->name }}</p>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="solar:calendar-date-bold"></iconify-icon>
                            <p class="text-xs">{{ date('d MM YY', strtotime($post[2]->date)) }}</p>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="heroicons:map-pin-20-solid"></iconify-icon>
                            <p class="text-xs">{{ $post[2]->location }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-3 col-span-12 space-y-6 w-full">
            <div class="w-full rounded-xl relative flex items-center gap-x-3">
                <img src="/assets/images/{{ $post[3]->cover }}" class="w-24 h-28 rounded-xl object-cover object-center relative" alt="Hallo Moshi Moshi">
                <div class="relative">
                    <div class="flex items-center gap-x-1">
                        <p class="text-xs">{{ $post[3]->category->name }}</p>
                    </div>
                    <h3 class="font-semibold text-md mb-2">{{ $post[3]->title }}</h3>
                    <div class="flex gap-x-3 items-center mb-2">
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="solar:calendar-date-bold"></iconify-icon>
                            <p class="text-xs">{{ date('d MM YY', strtotime($post[3]->date)) }}</p>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="heroicons:map-pin-20-solid"></iconify-icon>
                            <p class="text-xs">{{ $post[3]->location }}</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full rounded-xl relative flex items-center gap-x-3">
                <img src="/assets/images/{{ $post[4]->cover }}" class="w-24 h-28 rounded-xl object-cover object-center relative" alt="Hallo Moshi Moshi">
                <div class="relative">
                    <div class="flex items-center gap-x-1">
                        <p class="text-xs">{{ $post[4]->category->name }}</p>
                    </div>
                    <h3 class="font-semibold text-md mb-2">{{ $post[4]->title }}</h3>
                    <div class="flex gap-x-3 items-center mb-2">
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="solar:calendar-date-bold"></iconify-icon>
                            <p class="text-xs">{{ date('d MM YY', strtotime($post[4]->date)) }}</p>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="heroicons:map-pin-20-solid"></iconify-icon>
                            <p class="text-xs">{{ $post[4]->location }}</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full rounded-xl relative flex items-center gap-x-3">
                <img src="/assets/images/{{ $post[5]->cover }}" class="w-24 h-28 rounded-xl object-cover object-center relative" alt="Hallo Moshi Moshi">
                <div class="relative">
                    <div class="flex items-center gap-x-1">
                        <p class="text-xs">{{ $post[5]->category->name }}</p>
                    </div>
                    <h3 class="font-semibold text-md mb-2">{{ $post[5]->title }}</h3>
                    <div class="flex gap-x-3 items-center mb-2">
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="solar:calendar-date-bold"></iconify-icon>
                            <p class="text-xs">{{ date('d MM YY', strtotime($post[5]->date)) }}</p>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <iconify-icon class="text-lg" icon="heroicons:map-pin-20-solid"></iconify-icon>
                            <p class="text-xs">{{ $post[5]->location }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="flex w-full justify-center text-center mb-12 mt-8">
        <button class="btn bg-blue-900 hover:bg-blue-800 text-white btn-wide">Lebih Banyak</button>
    </div>
</div>

@if (count($youtube_media) > 0)
<div class="bg-blue-900 p-12 relative bg-fixed bg-center bg-cover" style="background-image: url('/assets/images/3.jpg')">
    <div class="absolute w-full left-0 top-0 h-full bg-blue-900/80"></div>
    <div class="text-white relative flex w-full justify-center text-center mb-12">
        <div>
            <p class="text-yellow-400 mb-4 tracking-wider uppercase md:pt-0 pt-12">KEGIATAN BERMUTU</p>
