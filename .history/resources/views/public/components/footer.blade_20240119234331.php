<div class="bg-blue-900 pt-12 relative bg-fixed bg-center bg-cover" style="background-image: url('/assets/images/4.jpg')">
            ...
            <div class="w-full grid p-8 grid-cols-1 relative justify-center justify-items-center">
                ...
                <div class="w-full bg-blue-900 p-8 mt-12 relative flex">
                    ...
                    <div class="lg:w-1/2 md:w-3/4 w-full relative">
                        ...
                        <label for="map_modal" class="flex items-center flex-wrap gap-x-3 my-4">
                            <iconify-icon icon="heroicons:map-pin-solid" class="text-2xl text-yellow-400"></iconify-icon>
                            <span for="map_modal" class="text-white">
                        ...
                        <div data-email="@foreach ($contact as $item) @if ($item->name == 'email') {{ $item->link }} @endif @endforeach" class="emaillink flex items-center flex-wrap gap-x-3 mb-4">
                    ...
                    <img src="/assets/images/{{ $content->contact_image }}" class="h-full object-cover object-center absolute right-0 bottom-0 lg:inline-block hidden" alt="Pak Harry Lawyer">
                </div>
            </div>
        </div>
    </div>
    <div class="relative md:px-24 px-12 py-32 grid lg:grid-cols-3 md:grid-cols-1 gap-6 items-center" id="footer">
        ...
        <div class="relative">
            ...
            <label for="map_modal" class="flex items-center gap-x-3 my-4">
                <iconify-icon icon="heroicons:map-pin-solid" class="text-2xl text-yellow-400"></iconify-icon>
                <span class="text-white">
                @foreach ($contact as $item)
                    @if ($item->name == 'alamat')
            ...
            <div role="button" data-email="@foreach ($contact as $item) @if ($item->name == 'email') {{ $item->link }} @endif @endforeach" class="emaillink1 flex items-center flex-wrap gap-x-3 mb-4">
                <iconify-icon icon="eva:email-outline" class="text-2xl text-yellow-400"></iconify-icon>
                <span class="text-white">
                @foreach ($contact as $item)
                    @if ($item->name == 'email')
    ...
    <div class="modal" role="dialog">
        <div class="modal-box max-w-3xl">
            ...
            <label for="map_modal" class="btn">Close!</label>
            </div>
        </div>
    </div>
    @foreach ($contact as $item)
...
document.querySelector('.emaillink').addEventListener('click', function() {
    var email = document.querySelector('.emaillink').getAttribute('data-email')
    var trim = email.trim()
    window.location.href = "mailto:"+trim
})
document.querySelector('.emaillink1').addEventListener('click', function() {
    var email = document.querySelector('.emaillink1').getAttribute('data-email')
    var trim = email.trim()
    window.location.href = "mailto:"+trim
})
