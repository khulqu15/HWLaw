...
document.getElementById('benefit_image').addEventListener('change', function(event) {
    if (event.target.files && event.target.files[0]) {
        let reader = new FileReader();
        reader.onload = function(e) {
            document.getElementById('benefit_image_preview').src = e.target.result;
            document.getElementById('benefit_image_preview').classList.remove('hidden');
        }
        reader.readAsDataURL(event.target.files[0]);
    }
})
...
document.getElementById('benefit_form').addEventListener('submit', function(event) {
    event.preventDefault()
    const benefit_image = document.querySelector('#benefit_image')
    const title = document.getElementById('benefit_title').value;
    const description = document.getElementById('benefit_description').value;
    let formData = new FormData()
    if(benefit_image.files[0]) formData.append('image2', benefit_image.files[0])
    formData.append('title', title)
    formData.append('description', description)
    fetch('/private/benefit/add', {
        method: 'POST',
        body: formData
    }).then(response => response.json())
    .then(data => {
