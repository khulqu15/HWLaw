...
@section('content')
    <div class="w-full bg-white p-8 relative">
        <form id="news_form" class="space-y-4">
            ...
            <button class="btn bg-indigo-600 hidden text-white" id="btn-cancel" onclick="window.location.reload()">Cancel</button>
...
coverInput.addEventListener('change', function(event) {
    if (event.target.files && event.target.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            document.getElementById("cover_label").setAttribute("for", "xyz")
...
document.getElementById('news_form').addEventListener('submit', function(e) {
    e.preventDefault()
    let croppedImage;
    if (cropper) croppedImage = cropper.getCroppedCanvas().toDataURL();
    ...
    formData.append('cover', croppedImage ? dataURItoBlob(croppedImage) : coverInput.files[0]);
